...
void initialize_radio()
{
  ...
  */
  ...
  const char *devAddr = "26011896";
  const char *nwkSKey = "A04813E4A58EE8076F3F3C29D87F7CD0";
  const char *appSKey = "28A13B809D55C62313078A3A9F2B8444";
...
void loop()
{
  ...
  attachInterrupt(digitalPinToInterrupt(3), throw_interruption, RISING);
  attachInterrupt(digitalPinToInterrupt(3), discard_interruption, FALLING);
...
float get_gasValue()
{
  ...
  float vol;
  int calibration = 216;
  vol=(float)gasValue/1024 * calibration;
  Serial.print("The gas density is ");
  Serial.println(vol);
  return vol;
}

void throw_interruption(){
  Serial.println("Gas value anormally high (over 50ppm)");
  led_on();
}

void discard_interruption(){
  Serial.println("Gas value return to normal");
  led_off();
}
